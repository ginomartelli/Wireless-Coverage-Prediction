...
def add_terrain_features(df):
    df["elevation"] = df.apply(
        lambda x: get_elevation(x["lat"], x["lon"]),
        axis=1
    )

    df["gw_elevation"] = df.apply(
        lambda x: get_elevation(x["gw_lat"], x["gw_lon"]),
        axis=1
    )
    ...
    df["terrain_type"] = df.apply(
        lambda x: get_terrain_type(x["lat"], x["lon"]),
        axis=1
    )
    ...
    df["slope"] = df.apply(
        lambda x: get_slope(x["lat"], x["lon"]),
        axis=1
    )

    df["roughness"] = df.apply(
        lambda x: get_roughness(x["lat"], x["lon"]),
        axis=1
    )

    df["obstruction_ratio"], df["max_obstruction"] = zip(*df.apply( #already computed in get_path_features
    ...
    terrain_features = pd.DataFrame(
        terrain_features.tolist()
    )
    df = pd.concat(
        [df.reset_index(drop=True),
        terrain_features.reset_index(drop=True)],
        axis=1
    )
    return df
...
def get_roughness(lat, lon):
    if(lat > 16.71):
        try:
            ...
            window = dem2.read(
                1,
                window=((row-2, row+3), (col-2, col+3))
            )
...
def get_obstruction_features(
    lat1, lon1, h1,
    lat2, lon2, h2, 
    distance,step_meters=30
):
...
def get_path_features(
    ...
    mean_obstruction = np.nanmean(
        np.maximum(obstruction_values, 0)
    )
