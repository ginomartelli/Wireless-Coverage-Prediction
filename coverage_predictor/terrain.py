...
# ── Configuration (externalised paths & constants) ──
...
dem = rasterio.open(os.path.join(TERRAIN_DIR, INF_CFG["dem1_tif"]))
dem2 = rasterio.open(os.path.join(TERRAIN_DIR, INF_CFG["dem2_tif"]))

terrain = gpd.read_file(os.path.join(TERRAIN_DIR, INF_CFG["landuse_geojson"]))
terrain2 = gpd.read_file(os.path.join(TERRAIN_DIR, INF_CFG["landuse2_geojson"]))
...
def get_terrain_type(lat, lon):
    ...
    dataset = terrain2 if _use_dem2(lat) else terrain

    matches = dataset[dataset.contains(point)]
...
def get_path_features(
    ...
    for i, (lat, lon) in enumerate(zip(lats, lons)):
        ...
        terrain_types.append(get_terrain_type(lat, lon))

        los_height = (
            h1_total
            +
            (h2_total - h1_total) * (i / (n_points - 1))
        )
    ...
    fresnel_obstruction_ratio = np.nanmean(fresnel_clearances < 0)
    ...
    residential_ratio = counts["residential"] / len(terrain_types)
